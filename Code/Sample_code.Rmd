...
stan_model <- stan_maker(Virus_counts,target_date, target_loc = "Colorado") # stan maker fits the non-mechanistic model, target loc allows you say where you want the model fit to, if Null it fits to all states that have enough seq in the last amount of days, the defaults are 90 seq in last 150 days form target date
stan_model_mech_nosv <- stan_maker_Mech(Virus_counts,target_date, target_loc = "New York", alt_file = "Mech_model_constant_nosv.stan" ) # stan_maker_Mech fits the mechanistic model
stan_model_mech_sv <- stan_maker_Mech(Virus_counts,target_date, target_loc = c("New York"), alt_file = "Mech_model_constant_sv.stan") # you can use alt_file to change the stan file, the defualt is the no sv model
